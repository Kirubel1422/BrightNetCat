...
class NetCat:
    ...
    def handle(self, client_socket):
        if self.args.execute:
            ...
            # Open up shell on listener
            while True:
                try:
                    # Prepare terminal for sender
                    client_socket.send(b'BHP: #> ')

                    # Read continously from sender
                    while '\n' not in cmd_buffer.decode():
                        cmd_buffer += client_socket.recv(64)
                    
                    # Save output of execution
                    response = execute(cmd_buffer.decode())

                    # Send response of execution to sender
                    if response:
                        client_socket.send(response.encode())
                    
                    # Restore cmd buffer state
                    cmd_buffer = b''
